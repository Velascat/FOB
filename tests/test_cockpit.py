...
class TestListRuns:
    def test_empty_root_returns_empty_list(self, tmp_path):
        from fob.runs import list_runs
...
class TestLatestRun:
    def test_returns_none_when_empty(self, tmp_path):
        from fob.runs import latest_run
...
class TestReadJson:
    def test_returns_empty_dict_for_missing_file(self, tmp_path):
        from fob.runs import read_json
...
class TestRunSummary:
    def test_includes_run_id(self, tmp_path):
        from fob.runs import run_summary
...
class TestDelegateArgParsing:
    def _parse(self, args):
        from fob.delegate import _parse_args
...
def test_runs_module_exists():
    assert (REPO_ROOT / "src" / "fob" / "runs.py").exists()


def test_last_module_exists():
    assert (REPO_ROOT / "src" / "fob" / "last.py").exists()


def test_delegate_module_exists():
    assert (REPO_ROOT / "src" / "fob" / "delegate.py").exists()


def test_system_status_module_exists():
    assert (REPO_ROOT / "src" / "fob" / "system_status.py").exists()


def test_cli_dispatches_delegate():
    text = (REPO_ROOT / "src" / "fob" / "cli.py").read_text()
